...
void Lab9::Update(float deltaTimeSeconds)
{
    ...
    // Why does this happen? How can you solve it?
    {
        ...
        RenderSimpleMeshEarth(meshes["sphere"], shaders["LabShader"], modelMatrix, mapTextures["earth"], mapTextures["earth"]);
    }
...
void Lab9::RenderSimpleMesh(Mesh *mesh, Shader *shader, const glm::mat4 & modelMatrix, Texture2D* texture1, Texture2D* texture2)
{
    ...
    GLint ceva = glGetUniformLocation(shader->program, "ciclare");
    glUniform1i(ceva, 0);

    GLint timp = glGetUniformLocation(shader->program, "time");
    glUniform1f(timp, Engine::GetElapsedTime());
...
void Lab9::RenderSimpleMeshEarth(Mesh* mesh, Shader* shader, const glm::mat4& modelMatrix, Texture2D* texture1, Texture2D* texture2)
{
    if (!mesh || !shader || !shader->GetProgramID())
        return;
